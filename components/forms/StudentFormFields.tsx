...
export const StudentFormFields = (p: Props) => {
    ...
    return <>
        ...
        <LabelAndInput readOnly={!p.canEdit} value={teacher} onChange={e => setTeacher(e.target.value)} label="Teacher" name="teacher" list="people" required />
        <LabelAndInput readOnly={!p.canEdit} value={student} onChange={e => setStudent(e.target.value)} label="Student" name="student" list="people" required />
        <LabelAndInput readOnly={!p.canEdit} value={source} onChange={e => setSource(e.target.value)} label="Source" name="source" required />
    </>;
};
